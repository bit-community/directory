...
export const ProfileCard = (props) => {
    const { fullName, _id, skills, yearsOfExperience, countryOfResidence, professionalTitle, profilePhoto, } = props;
    const history = useHistory();
    const viewProfile = (id) => {
        history.push(path.baddie + '/' + id);
    };
    return (<CardWrapper flexDirection="column" onClick={() => viewProfile(_id)} justifyContent="space-between" position="relative" p={4}>
      ...
      <Stack spacing={2} isInline pt={2} mb={4}>
        ...
        return (<Tag key={[index, val].join('-')} rounded="full" size="sm" bg="#E4E7F9">
...
export const DirectoryPage = (props) => {
    ...
    if (!props.profiles || props.profiles.length === 0) {
        return (<Flex align="center" justify="center" margin="auto">
        <Loader />
      </Flex>);
    }
    const useFuse = (value) => {
    ...
    return (<Box>
      ...
      <Wrapper>
        ...
        <InputGroup color="blue.800" width="100%" maxWidth="600px" size="lg" my="5">
          ...
          <Input variant="outline" placeholder="Search by name or role" color="blue.800" borderRadius="5px" focusBorderColor="blue.800" onChange={(e) => handleSearch(e)}/>
        </InputGroup>
        ...
        ? result.map((res, _index) => {
            const { item } = res;
            return (<ProfileCard key={[item._id, _index].join('-')} fullName={item.fullName} profilePhoto={item.profilePhoto} professionalTitle={item.professionalTitle || ''} yearsOfExperience={item.yearsOfExperience} professionalBio={item.professionalBio} skills={item.skills} _id={item._id} cityOrState={item.cityOrState} countryOfResidence={item.countryOfResidence}/>);
        })
        : profiles.map((val, _index) => {
            return (<ProfileCard key={[val._id, _index].join('-')} fullName={val.fullName} profilePhoto={val.profilePhoto} professionalTitle={val.professionalTitle || ''} yearsOfExperience={val.yearsOfExperience} professionalBio={val.professionalBio} skills={val.skills} _id={val._id} cityOrState={val.cityOrState} countryOfResidence={val.countryOfResidence}/>);
        })}
        </Flex>
      </Wrapper>
    </Box>);
